...
<nav className="flex items-center justify-between bg-[#00234E] text-white p-4 shadow-md fixed top-0 left-0 right-0 z-50">
      ...
      <div className="flex space-x-6">
        ...
        <div className="w-4/5 border-r pr-6 h-[600px] overflow-y-auto">
  ...
  <tbody>
    {Object.entries(documentData.document_fields || {})
      ...
      {openContainer === "distributor" && (
        <div className="mt-4 border p-4 rounded bg-white relative">
          {/* Close Icon */}
          <button
          ...
          <ul className="space-y-2">
            {distributors.map((dist) => (
              <li key={dist.user_id} className="border-b last:border-b-0">
                <button
                  onClick={() => handleAssignDistributor(dist.user_id)}
                  className="w-full text-left p-2 border-2 rounded hover:bg-blue-100"
                >
                  {dist.name}
                </button>
              </li>
...
{selectedDocument && (
  <div className="fixed inset-0 flex items-center justify-center z-50">
    <div className="relative w-3/4 md:w-2/3 lg:w-1/2 h-3/4 bg-gray-100 rounded-lg p-4">
      {/* Close Button */}
      <button
