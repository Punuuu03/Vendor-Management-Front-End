...
const CompletedApplicationsList = () => {
    ...
    // Fetch completed documents based on user_id
    useEffect(() => {
        if (!userId) return;

        const fetchCompletedDocuments = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/userdashboard/completed/${userId}`);
                console.log("Fetched completed documents:", response.data);
                setCompletedDocuments(response.data);
            } catch (error) {
    ...
    // Filter completed documents based on search query
        ...
        <div className="ml-[260px] flex flex-col items-center min-h-screen p-10 bg-gray-100">
            <div className="w-full p-6">
                <div className="w-full max-w-7xl bg-white p-6 shadow-lg">
                    <h2 className="text-2xl font-bold mb-4">Completed Applications</h2>
